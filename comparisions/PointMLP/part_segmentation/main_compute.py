...
def train(args, io):
    ...
    # =========== Dataloader =================
    npoints = 102400
    train_data = PartNormalDataset(npoints=npoints, split='trainval', normalize=False)
    print("The number of training data is:%d", len(train_data))

    test_data = PartNormalDataset(npoints=npoints, split='test', normalize=False)
...
def train_epoch(train_loader, model, opt, scheduler, epoch, num_part, num_classes, io):
    ...
    for batch_id, (points, label, target, norm_plt) in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
        ...
        # 计算FLOPS和参数量
        flops, params = thop.profile(model, inputs=(points,norm_plt, to_categorical(label, num_classes)))
...
def test_epoch(test_loader, model, epoch, num_part, num_classes, io):
    ...
    final_total_per_cat_iou = np.zeros(1).astype(np.float32)
    # final_total_per_cat_seen = np.zeros(16).astype(np.int32)
    final_total_per_cat_seen = np.zeros(1).astype(np.int32)
    metrics = defaultdict(lambda: list())
    model.eval()
...
def test(args, io):
    ...
    total_per_cat_iou = np.zeros((1)).astype(np.float32)
    # total_per_cat_seen = np.zeros((16)).astype(np.int32)
    total_per_cat_seen = np.zeros((1)).astype(np.int32)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_name', type=str, default='demo_v3_compute', metavar='N',
    ...
    parser.add_argument('--num_points', type=int, default=102400,
