...
def parse_args():
    ...
    parser.add_argument('--gpu', type=str, default='3', help='specify GPU devices')
    ...
    parser.add_argument('--ckpts', type=str, default='../checkpoints/pretrain.pth', help='ckpts')
    parser.add_argument('--root', type=str, default='../data/origin_segment_all_v3/', help='data root')
...
## we use adamw and cosine scheduler
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target,filepath) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            # points, label, target = points.float().cuda(), label.float().cuda(), target.long().cuda()
            points = points.transpose(2, 1)
            # 计算FLOPS和参数量
            flops, params = thop.profile(classifier, inputs=(points, label))
