...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, required=False,default='pointnet2_part_seg_msg_v3', help='experiment root')
...
def main(args):
    ...
    with torch.no_grad():
        ...
        for batch_id, (points, label, target,filepath) in tqdm(enumerate(testDataLoader), total=len(testDataLoader),
            ...
            # 转换为 CPU 格式并进行数值计算
            points = points.transpose(2, 1).cpu().numpy().reshape(-1, 6)  # 转回原始形状并转换为 NumPy (x, y, z, nx, ny, nz)
            target = target.cpu().numpy().flatten()  # 获取实际标签

            # 将点云和预测标签拼接在一起，保存格式为 x, y, z, nx, ny, nz, predicted_label
            ...
            # 原始的也保留
            # np.savetxt(save_filepath_origin, origin_pointscloud, fmt='%.6f %.6f %.6f %.6f %.6f %.6f %.6f', delimiter=' ')
