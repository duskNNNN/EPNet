...
class get_model(nn.Module):
    def __init__(self, cls_dim):
        ...
        self.label_conv = nn.Sequential(nn.Conv1d(1, 64, kernel_size=1, bias=False),
                                        nn.BatchNorm1d(64),
                                        nn.LeakyReLU(0.2))
        ...
        self.convs1 = nn.Conv1d(3776, 512, 1)
    ...
    def forward(self, pts, cls_label):
        ...
        feature_list, cls = self.blocks(x, pos)
        cls = cls.max(1)[0]
        ...
        cls_label_one_hot = cls_label.view(B, 1, 1)
        cls_label_feature = self.label_conv(cls_label_one_hot).repeat(1, 1, N)

        cls = cls.unsqueeze(2).repeat(1, 1, N)
        x_global_feature = torch.cat((x_max_feature, x_avg_feature, cls, cls_label_feature), 1)  # 1152*2 + 384 + 64
