...
def parse_args():
    ...
    parser.add_argument('--cfg',type=str,default='config/queue/new_res_select_v8_augument_v1_factor_test.json',help='config file')
...
def main(args):
    ...
    trainDataLoader = torch.utils.data.DataLoader(TRAIN_DATASET, batch_size=int(cfg["batch_size"]),pin_memory=True,
    ...
    classifier = MODEL.get_model(npoints,num_classes,num_part, normal_channel=int(cfg["is_normal"]))
    ...
    for epoch in range(start_epoch, int(cfg["epoch"])):
        ...
        log_string('Learning rate:%f' % cur_lr)
        classifier = classifier.train()
        writer.add_scalar('Learning Rate', cur_lr, global_epoch) 
        lr_schedule.step()
        total_train_loss = 0
        '''learning one epoch'''
