...
class SetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        grouped_xyz = index_points(xyz, idx) # [B, npoint, nsample, C]
        grouped_points = index_points(points, idx)
        grouped_xyz_selected, grouped_points_selected = select_topk_by_cosine_similarity(grouped_xyz, grouped_points,round(2/5 * nsample), new_xyz)
...
def sample_and_group_new(npoint, nsample, xyz, points):
    ...
    grouped_xyz = index_points(xyz, idx) # [B, npoint, nsample, C]
    grouped_points = index_points(points, idx)
    grouped_xyz_selected, grouped_points_selected = select_topk_by_cosine_similarity(grouped_xyz, grouped_points,round(5/6 * nsample), new_xyz)
    grouped_xyz_norm = grouped_xyz_selected - new_xyz.view(B, S, 1, C)
    new_points = torch.cat([grouped_xyz_norm, grouped_points], dim=-1) # [B, npoint, nsample, C+D]
...
class PointNetSetAbstractionMsgSelectNormal(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        for i in range(len(self.nsample_list)):
            ...
            grouped_xyz = index_points(xyz, group_idx)
            grouped_points = index_points(points, group_idx)
            grouped_xyz_selected, grouped_points_selected,selected_normals = select_topk_by_cosine_similarity_normal(grouped_xyz, grouped_points,round(2/5 * K), new_xyz)
